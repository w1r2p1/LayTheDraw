...
# write back_lay = 'BACK' or 'LAY'
def placeBet(marketId, selectionId,back_lay,bet_amount,odds):
    if( marketId is not None and selectionId is not None):
        ...
        place_order_Req = '{"jsonrpc": "2.0", "method": "SportsAPING/v1.0/placeOrders", "params": {"marketId":"' + marketId + '","instructions":'\
                                                                                                                              '[{"selectionId":"' + str(
            selectionId) + '","side":"' + back_lay + '","orderType":"LIMIT","limitOrder":{"size":"' + bet_amount + '","price":"' + odds + '","persistenceType":"LAPSE"}}]}, "id": 1}'#,"customerRef":"test12121212121" inside second last }
        """
        ...
        try:
			...
			try:
				if str(place_order_load['result']['errorCode']) == 'INSUFFICIENT_FUNDS':
					print "Insufficient funds"
				else:
					print  'Exception from API-NG result: '  + str(place_order_load['result']) + " jsonrpc: " + str(place_order_load['jsonrpc']) #['errorCode'])
			except KeyError:
					print  'Exception from API-NG result: '  + str(place_order_load['result']) + " jsonrpc: " + str(place_order_load['jsonrpc']) 
	return place_order_result['status']
